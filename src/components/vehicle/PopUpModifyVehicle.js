...
export default function PopUpModifyVehicle({
	...
	const confirmMutation = async (mutation) => {
		...
		try {
			await mutation({
				variables: {
					updateVehicleId: valueModify.id,
					vehicle: { ...dataAfterModification, image: imageUrl },
				},
			});
		} catch (error) {
			setToast({
				header: 'Erreur',
				body: `${JSON.stringify(
					error.networkError.result.errors[0].message || 'Erreur Server'
				)}`,
				type: 'danger',
				state: true,
			});
              ...
              <Row className="mt-3 mb-3">
				...
				<div className='p-field p-col-12 p-md-6'>
                  <Controller
						...
						render={({ field }) => (
							<>
							<InputLabel id="demo-simple-select-standard-label">Chauffeur à assigner</InputLabel>
							<Select
							fullWidth
							...
							>
								{GetDrivers().map(({value, label})=><MenuItem value={value}>{label}</MenuItem>)}
							</Select>
							</>
						)}
					/>
				 ...
				 <Col>
					<div className='p-field p-col-6'>
						<Controller
							...
							render={({ field }) => (
								<>
								<InputLabel id="demo-simple-select-standard-label">Année de debut *</InputLabel>
								<Select
								labelId="demo-simple-select-label"
								id="demo-simple-select"
								value={field.value}
								...
								>
									{getYears.map(({value, label})=><MenuItem value={value}>{label}</MenuItem>)}
								</Select>
								</>
							)}
						/>
						...
						<Controller
							...
							render={({ field }) => (
								<>
									<TextField
										fullWidth
										label="Nom du Vehicule *" variant="outlined"
										id={field.name}
										error={errors.name}
										{...field}
										value={field.value}
						...
						<Controller
							...
							render={({ field }) => (
								<>
									<TextField
										fullWidth
										label="Model de vehicule *" variant="outlined"
										id={field.name}
										{...field}
										value={field.value}
										type='text'
					...
					<div className='p-field p-col-6'>
						<Controller
							...
							render={({ field }) => (
								<>
									<TextField
										label={"Numero de serie *"} variant="outlined"
										id={field.name}
										fullWidth
										error={errors.serie}
										{...field}
					...
					<div className='p-field p-col-12 p-md-6'>
						<Controller
							...
							render={({ field }) => (
								<>
									<TextField
										label="Numero d'enregistrement *" variant="outlined"
										id={field.name}
						...
						{errors.registrationNumber &&
							errors.registrationNumber.type === 'required' && (
								<small className='alert-danger p-1'>
									Le numero d'enregistrement est requis !
								</small>
							)}
					</div>
				</Col>
					...
					<div className='p-field p-col-12 p-md-6'>
						<Controller
							...
							render={({ field }) => (
								<>
									<TextField
										label="Puissance du moteur" variant="outlined"
										id={field.name}
										error={errors.power}
										fullWidth
										type='text'
			  ...
			  <Row className="mt-3 mb-3">
				  <Col>
				  	<Form.Group controlId='formFileSm' className='mb-3'>
						<label htmlFor='address'>Photo du vehicule</label>
