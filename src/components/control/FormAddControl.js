...
export default function FormAddControl({ idVehicle, dateControl }) {
								...
								<Grid item xl={3} lg={3} sm={6} xs={12}>
									<span className='p-float-label'>
										<Controller
											...
											render={({ field }) => (
												<>
													<span>{GetFrenchElementControl(field.name)}</span><br />
													<InputText
														type='number'
														style={{width: '100%'}}
														value={field.value}
	...
	return (
		...
		<Card>
        <CardHeader
          title={'Un controle existe déjà à la date donné'}
        />
        <Divider />
        <CardContent>
          <Box
